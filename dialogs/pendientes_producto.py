...
class PendientesDeAprobacion(QDialog):
    ...
    def mostrar_productos(self, productos: list):
        ...
        for i, p in enumerate(productos):
            ...
            self.tabla.setItem(i, 2, QTableWidgetItem(p["codigo_barra"]))
            self.tabla.setItem(i, 3, QTableWidgetItem(str(p["stock_actual"])))
            self.tabla.setItem(i, 4, QTableWidgetItem(str(p["fecha_creacion"])))
    ...
    def aprobar_producto(self, fila, _col):
        codigo = self.tabla.item(fila, 2).text()
        item = self.tabla.item(fila, 0)
        item_id = item.data(Qt.ItemDataRole.UserRole)
        dialogo = AprobarProductoDialog(self.sesion, self.config_sistema, codigo, item_id)
        if dialogo.exec():
            self.cargar_productos()
